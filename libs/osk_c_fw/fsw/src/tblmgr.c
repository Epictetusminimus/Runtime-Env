...
static bool LoadTblStub(TBLMGR_Tbl *Tbl, uint8 LoadType, const char* Filename);
static bool DumpTblStub(TBLMGR_Tbl *Tbl, uint8 DumpType, const char* Filename);
...
bool TBLMGR_LoadTblCmd(void* ObjDataPtr, const CFE_SB_Buffer_t* SbBufPtr)
{
   ...
   const  TBLMGR_LoadTblCmdMsg *LoadTblCmd = (const TBLMGR_LoadTblCmdMsg *) SbBufPtr;
...
bool TBLMGR_DumpTblCmd(void* ObjDataPtr, const CFE_SB_Buffer_t* SbBufPtr)
{
   ...
   TBLMGR_Tbl   *Tbl;
   TBLMGR_Class *TblMgr = (TBLMGR_Class *) ObjDataPtr;
   const  TBLMGR_DumpTblCmdMsg *DumpTblCmd = (const TBLMGR_DumpTblCmdMsg *) SbBufPtr;
