...
static bool        ConfigCallback (void* UserData, int TokenIdx);  /* Must match JSON_ContainerFuncPtr */
static const char* ConfigTypeStr(INITBL_Type Type);
static bool        LoadConfigurations(INITBL_Class* IniTbl, const char* IniFile);
static bool        ValidConfigParam(INITBL_Class* IniTbl, uint16 Param, INITBL_Type Type);
...
**
*/
bool INITBL_Constructor(INITBL_Class* IniTbl, const char* IniFile,
                        INILIB_CfgEnum* CfgEnum)
{
   ...
   if ((IniTbl->CfgEnum.End-2) < INITBL_MAX_CFG_ITEMS) {
      ...
      CFE_EVS_SendEvent(INITBL_CONFIG_DEF_ERR_EID, CFE_EVS_EventType_ERROR, "JSON INITBL definition error. JSON config file contains % d which is greater than frame maximum defined at %d",
...
static bool LoadConfigurations(INITBL_Class* IniTbl, const char* IniFile)
{
   ...
   if (JSON_OpenFile(JSON, IniFile)) {
      ...
      if (IniTbl->AttrErrCnt == 0) {
         ...
         CFE_EVS_SendEvent(INITBL_LOAD_CONFIG_ERR_EID, CFE_EVS_EventType_ERROR, "LoadConfigurations() failed with %d JSON attribute errors",
                           IniTbl->AttrErrCnt);
      ...
      CFE_EVS_SendEvent(INITBL_LOAD_CONFIG_ERR_EID, CFE_EVS_EventType_ERROR, "FileMgr::LoadConfigurations() Error preparing JSON file %s\n", IniFile);
...
**
*/
static bool ConfigCallback (void* UserData, int TokenIdx)
{
   ...
   if (IniTbl->JsonVarCnt == (IniTbl->CfgEnum.End-1)) {
      ...
      RetStatus = true;
      CFE_EVS_SendEvent(INITBL_JSON_PARSE_EID, CFE_EVS_EventType_INFORMATION, "Successfully configured %d initialization file attributes",
                        IniTbl->JsonVarCnt);
      ...
      ++IniTbl->AttrErrCnt;     
      CFE_EVS_SendEvent(INITBL_JSON_PARSE_ERR_EID, CFE_EVS_EventType_ERROR, "Invalid number of packet attributes %d. Should be %d.",
   ...
   if ( Param > IniTbl->CfgEnum.Start && Param < IniTbl->CfgEnum.End) {
      ...
      if (IniTbl->CfgItem[Param].Initialized) {
         if (IniTbl->CfgItem[Param].Type == Type) {
            ...
            CFE_EVS_SendEvent(INITBL_CFG_PARAM_ERR_EID, CFE_EVS_EventType_ERROR, "Attempt to retrieve parameter of type %s that was loaded as type %s",
                              ConfigTypeStr(Type),ConfigTypeStr(IniTbl->CfgItem[Param].Type));      
         }
      }
      else {
         CFE_EVS_SendEvent(INITBL_CFG_PARAM_ERR_EID, CFE_EVS_EventType_ERROR, "Attempt to retrieve uninitialized parameter %d", Param);
      }         
   }
   else {
